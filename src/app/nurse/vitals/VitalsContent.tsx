...
import { Activity, Thermometer, Eye, Heart, CheckCircle2 } from "lucide-react";
      ...
      {preselectedName && (
        <div className="rounded-xl bg-cyan-50 border border-cyan-200 p-4 flex items-center gap-3">
          ...
          <p className="text-sm text-cyan-800">
            Ready to record vitals for <span className="font-bold">{preselectedName}</span>. Click <strong>Record</strong> in the table below to begin.
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-4 md:gap-8 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader className="flex-row items-center justify-between">
            <CardTitle>Pending Vitals</CardTitle>
              ...
              <Table>
                ...
                <TableBody>
                  {pendingVitals.map((v) => (
                    <TableRow
                      key={v.id}
                      className={v.patientId === preselectedPatientId ? "bg-cyan-50/50" : ""}
                    >
                      ...
                      <TableCell className="text-center text-slate-400 tabular-nums">
                        {v.temperature}
                      </TableCell>
                      <TableCell className="text-center">
                        <span className="inline-flex items-center gap-1 text-slate-400 tabular-nums">
                      ...
                      <TableCell className="text-right">
                        <button
                          onClick={() => openRecordingModal(v.patientId)}
              ...
              <TableBody>
                {waitingPatients.map((p) => (
                  <TableRow key={p.id}>
                    ...
                    <TableCell className="text-right">
                      <button
                        onClick={() => openRecordingModal(p.patientId)}
