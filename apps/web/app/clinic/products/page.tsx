...
const emptyForm = { name: "", category: "", unitPrice: 0, quantityInStock: "" as string | number, description: "", reorderLevel: "" as string | number };
...
export default function ClinicProductsPage() {
  ...
  const stats = useMemo(() => ({
    ...
    tracked: products.filter((p) => p.quantityInStock !== null).length,
  }), [products, categories]);
  ...
  const handleOpenEdit = (product: Product) => {
    ...
    setStockUnlimited(product.quantityInStock === null);
    setFormError("");
  ...
  const handleSave = () => {
    ...
    const data = {
      ...
      quantityInStock: stockUnlimited ? undefined : Math.max(0, Number(form.quantityInStock) || 0),
      reorderLevel: form.reorderLevel ? Number(form.reorderLevel) : undefined,
      description: form.description.trim() || undefined,
    };

    if (editingProduct) {
      updateMut.mutate({ id: editingProduct.id, dto: data }, {
        onSuccess: () => { setShowModal(false); setEditingProduct(null); },
      });
    } else {
  ...
  const handleDelete = () => {
    if (deletingProduct) {
      deleteMut.mutate(deletingProduct.id, {
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      {/* Add / Edit Modal */}
      <Modal isOpen={showModal} onClose={() => { setShowModal(false); setEditingProduct(null); }} title={editingProduct ? "Edit Product" : "Add Product"}>
        <div className="space-y-5">
          <div>
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1.5">Product Name <span className="text-rose-500">*</span></label>
            <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
              placeholder="e.g. Blue Light Filter, Comprehensive Eye Exam"
              className="w-full h-11 px-4 rounded-xl bg-slate-50 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 transition-all text-sm font-bold text-slate-900 placeholder:text-slate-300" autoFocus />
          </div>
          <div>
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1.5">Category <span className="text-rose-500">*</span></label>
            <input type="text" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}
              placeholder="e.g. Optical, Pharmacy, Consultation" list="clinic-product-categories"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1.5">Unit Price (&#x20A6;) <span className="text-rose-500">*</span></label>
              <input type="number" min={0} step={100} value={form.unitPrice || ""} onChange={(e) => setForm({ ...form, unitPrice: Math.max(0, Number(e.target.value)) })}
                placeholder="0" className="w-full h-11 px-4 rounded-xl bg-slate-50 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 transition-all text-sm font-bold text-slate-900 placeholder:text-slate-300" />
            </div>
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1.5">Stock</label>
              <div className="flex items-center gap-3">
                <input type="number" min={0} value={stockUnlimited ? "" : (form.quantityInStock ?? "")}
                  onChange={(e) => setForm({ ...form, quantityInStock: Math.max(0, Number(e.target.value)) })}
                  placeholder="Qty" disabled={stockUnlimited}
                  className={cn("w-full h-11 px-4 rounded-xl border transition-all text-sm font-bold text-slate-900 placeholder:text-slate-300",
                    stockUnlimited ? "bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed" : "bg-slate-50 border-slate-200 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500")} />
                <label className="flex items-center gap-1.5 shrink-0 cursor-pointer select-none">
                  <input type="checkbox" checked={stockUnlimited}
                    onChange={(e) => { setStockUnlimited(e.target.checked); if (e.target.checked) setForm({ ...form, quantityInStock: "" }); }}
          ...
          <div>
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1.5">Description <span className="text-slate-300 font-normal normal-case tracking-normal">(optional)</span></label>
            <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
              placeholder="Brief description or notes about this product..." rows={2}
          ...
          <div className="flex items-center gap-3 pt-2">
            <Button onClick={() => { setShowModal(false); setEditingProduct(null); }} variant="outline" className="flex-1 h-11 rounded-xl font-bold border-slate-200">Cancel</Button>
            <Button onClick={handleSave} disabled={createMut.isPending || updateMut.isPending} variant="default" className="flex-1 h-11 rounded-xl font-bold gap-2 bg-sky-600 hover:bg-sky-700 shadow-lg shadow-sky-600/20 disabled:opacity-50">
      ...
      <Modal isOpen={!!deletingProduct} onClose={() => setDeletingProduct(null)} title="Delete Product">
        <div className="space-y-5">
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-sm font-bold text-slate-900">Delete <span className="text-sky-600">{deletingProduct?.name}</span>?</p>
              <p className="text-xs text-slate-500 mt-1">This removes it from the POS catalog. Completed transactions are not affected.</p>
            </div>
          </div>
          <div className="flex items-center gap-3 pt-2">
            <Button onClick={() => setDeletingProduct(null)} variant="outline" className="flex-1 h-11 rounded-xl font-bold border-slate-200">Cancel</Button>
            <Button onClick={handleDelete} disabled={deleteMut.isPending} variant="default" className="flex-1 h-11 rounded-xl font-bold gap-2 bg-rose-600 hover:bg-rose-700 shadow-lg shadow-rose-600/20 disabled:opacity-50">
              {deleteMut.isPending ? <Loader2 size={16} className="animate-spin" /> : <Trash2 size={16} />} Delete
            </Button>
          </div>
        </div>
      </Modal>
