...
import { useHmoClaims, useCreateAppeal, useUpdateAppeal } from "@/hooks/useHmo";
import {
  Search, Plus, ShieldAlert, CheckCircle2,
  AlertCircle, Send, Clock, X, FileText,
...
interface LocalAppeal {
  ...
  status: "draft" | "submitted" | "under_review" | "approved" | "dismissed";
...
// Local appeals state (backend has no list endpoint)
const LOCAL_APPEALS_KEY = "vemtap-hmo-appeals";

function loadLocalAppeals(): LocalAppeal[] {
...
function saveLocalAppeals(appeals: LocalAppeal[]) {
...
export default function AppealsPage() {
  ...
  const createAppealMut = useCreateAppeal();
  const updateAppealMut = useUpdateAppeal();

  const [appeals, setAppeals] = useState<LocalAppeal[]>(() => loadLocalAppeals());
  ...
  const queriedClaims = claims.filter((c) => c.status === "denied" || c.status === "appealed");
  ...
  const persistAppeals = (updated: LocalAppeal[]) => {
    setAppeals(updated);
    saveLocalAppeals(updated);
  };

  const handleAddAppeal = () => {
    if (!validateForm()) return;
    const newAppeal: LocalAppeal = {
      id: `APL-${String(appeals.length + 1).padStart(3, "0")}`,
    ...
    persistAppeals([...appeals, newAppeal]);
    setForm({ ...emptyForm });
    setFormErrors({});
  ...
  const handleQuickCreate = (claimId: string) => {
    ...
    setForm({
      ...
      hmoName: c.hmo?.name || c.hmoId,
      patientName: c.patient ? `${c.patient.firstName} ${c.patient.lastName}` : c.patientId,
      reason: `Appeal for ${c.claimNumber} \u2014 ${c.diagnosis || "N/A"}`,
      supportingNotes: "",
    });
    setFormErrors({});
  ...
  const handleDismiss = () => {
    ...
    const updated = appeals.map((a) =>
      a.id === showDismissConfirm ? { ...a, status: "dismissed" as const, responseDate: new Date().toISOString().slice(0, 10), response: "Appeal dismissed." } : a
    );
    persistAppeals(updated);
  ...
  const handleApprove = (id: string) => {
    const updated = appeals.map((a) =>
      a.id === id ? { ...a, status: "approved" as const, responseDate: new Date().toISOString().slice(0, 10), response: "Appeal approved \u2014 claim reinstated." } : a
    );
    persistAppeals(updated);
    setShowDetail(null);
    showNotification("success", "Appeal approved.");
  };

  const handleSubmit = (id: string) => {
    const updated = appeals.map((a) =>
      a.id === id ? { ...a, status: "submitted" as const, submittedDate: new Date().toISOString().slice(0, 10) } : a
    );
    persistAppeals(updated);
    setShowDetail(null);
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      {/* Add Appeal Modal */}
      <Modal isOpen={showAddModal} onClose={() => setShowAddModal(false)} title="New Appeal">
        <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="text" value={form.claimId} onChange={(e) => { setForm({ ...form, claimId: e.target.value }); clearFieldError("claimId"); }}
                  placeholder="e.g. CLM-001" list="appeal-claim-ids"
                ...
                <datalist id="appeal-claim-ids">
                  {queriedClaims.map((c) => <option key={c.id} value={c.claimNumber} />)}
                </datalist>
          ...
          <div className="flex items-center gap-3 pt-1">
            <Button onClick={() => setShowAddModal(false)} variant="outline" className="flex-1 h-11 rounded-xl font-bold border-slate-200">Cancel</Button>
            <Button onClick={handleAddAppeal} variant="default" className="flex-1 h-11 rounded-xl font-bold gap-2 bg-sky-600 hover:bg-sky-700 disabled:opacity-50">
              <ShieldAlert size={16} /> Create Appeal
            </Button>
          </div>
        </div>
      </Modal>
            ...
            <div className="flex items-center gap-3 pt-1">
              {detail.status === "draft" && (
                <Button onClick={() => handleSubmit(detail.id)}
                  variant="default" className="flex-1 h-11 rounded-xl font-bold gap-2 bg-sky-600 hover:bg-sky-700 disabled:opacity-50">
              ...
              {detail.status === "submitted" && (
                <>
                  <Button onClick={() => handleApprove(detail.id)}
                    variant="default" className="flex-1 h-11 rounded-xl font-bold gap-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50">
                    <CheckCircle2 size={16} /> Approve Appeal
                  </Button>
                  <Button onClick={() => setShowDismissConfirm(detail.id)}
                    variant="default" className="flex-1 h-11 rounded-xl font-bold gap-2 bg-rose-600 hover:bg-rose-700 disabled:opacity-50">
                    <X size={16} /> Dismiss
                  </Button>
                </>
              )}
        ...
        <div className="space-y-4">
          <div className="flex items-start gap-3 p-4 rounded-xl bg-rose-50 border border-rose-200">
            ...
            <div>
              ...
              <p className="text-xs text-rose-700 mt-1">This will mark {showDismissConfirm} as dismissed.</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <Button onClick={handleDismiss} variant="default" className="flex-1 h-11 rounded-xl font-bold gap-2 bg-rose-600 hover:bg-rose-700">
              <X size={16} /> Dismiss Appeal
            </Button>
          </div>
        </div>
      </Modal>
