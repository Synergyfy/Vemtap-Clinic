...
import { useHmoClaims, useHmoRemittances, useCreateRemittance, useMatchRemittance } from "@/hooks/useHmo";
import {
  Search, Plus, CheckCircle2, AlertCircle,
  DollarSign, X, Loader2, Banknote
} from "lucide-react";
...
export default function ReconciliationPage() {
  const { user } = useAuth();
  const { data: remittances = [], isLoading: remittancesLoading } = useHmoRemittances();
  const { data: claims = [] } = useHmoClaims();
  const createRemittanceMut = useCreateRemittance();
  const matchRemittanceMut = useMatchRemittance();
  ...
  const unmatchedClaims = claims.filter((c) => c.status === "approved" || c.status === "settled");

  const filtered = remittances.filter((r) =>
    !search || r.remittanceNumber.toLowerCase().includes(search.toLowerCase()) || (r.hmo?.name || "").toLowerCase().includes(search.toLowerCase())
  );
  ...
  const handleAddRemittance = () => {
    ...
    createRemittanceMut.mutate({
      remittanceNumber: form.reference,
      ...
      onError: () => showNotification("error", "Failed to record remittance"),
    });
  };

  const handleMatch = () => {
    if (showMatchModal && selectedMatchIds.size > 0) {
      matchRemittanceMut.mutate({ id: showMatchModal, claimIds: Array.from(selectedMatchIds) }, {
        onSuccess: () => {
          setSelectedMatchIds(new Set());
          setShowMatchModal(null);
          showNotification("success", `${selectedMatchIds.size} claim(s) matched successfully.`);
        },
        onError: () => showNotification("error", "Failed to match claims"),
      });
    }
  };
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      {(createRemittanceMut.isPending || matchRemittanceMut.isPending) && (
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card className="border-none shadow-sm rounded-2xl">
          <CardContent className="p-5">
            ...
            <p className="text-xl font-black text-slate-900 mt-1">{formatNGN(remittances.reduce((s, r) => s + r.totalAmount, 0))}</p>
          </CardContent>
        </Card>
        <Card className="border-none shadow-sm rounded-2xl">
          <CardContent className="p-5">
        ...
        <Card className="border-none shadow-sm rounded-2xl">
          <CardContent className="p-5">
            ...
            <p className="text-xl font-black text-amber-600 mt-1">{remittances.filter((r) => r.status === "pending").length}</p>
          </CardContent>
        </Card>
      </div>
      ...
      {remittancesLoading ? (
        <Card className="border-none shadow-sm rounded-2xl">
          <CardContent className="p-16 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-sm font-bold text-slate-400">Loading remittances...</p>
          </CardContent>
        </Card>
              ...
              <tbody className="divide-y divide-slate-50">
                {filtered.map((r) => (
                  <tr key={r.id} className="group hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 text-sm font-bold text-slate-900">{r.remittanceNumber}</td>
                    <td className="px-6 py-4 text-sm text-slate-700">{r.hmo?.name || r.hmoId}</td>
                    <td className="px-6 py-4 text-sm font-black text-slate-900">{formatNGN(r.netAmount)}</td>
                    ...
                    <td className="px-6 py-4">
                      <Badge className={cn("text-[10px] font-black uppercase tracking-widest border-0",
                        ...
                        r.status === "disputed" ? "bg-rose-50 text-rose-700" :
      ...
      <Modal isOpen={showAddModal} onClose={() => !createRemittanceMut.isPending && setShowAddModal(false)} title="Record Remittance">
        <div className="space-y-5">
          ...
          <div className="flex items-center gap-3 pt-1">
            <Button onClick={() => setShowAddModal(false)} disabled={createRemittanceMut.isPending} variant="outline" className="flex-1 h-11 rounded-xl font-bold border-slate-200">Cancel</Button>
            <Button onClick={handleAddRemittance} disabled={createRemittanceMut.isPending} variant="default" className="flex-1 h-11 rounded-xl font-bold gap-2 bg-sky-600 hover:bg-sky-700 disabled:opacity-50">
              {createRemittanceMut.isPending ? <Loader2 size={16} className="animate-spin" /> : <DollarSign size={16} />}
              {createRemittanceMut.isPending ? "Recording..." : "Record Remittance"}
            </Button>
          </div>
        </div>
      </Modal>
      ...
      {/* Match Claims Modal */}
      <Modal isOpen={!!showMatchModal} onClose={() => !matchRemittanceMut.isPending && setShowMatchModal(null)} title={remittanceMatch ? "Match Claims" : ""}>
        {remittanceMatch && (
          <div className="space-y-5">
            <div className="grid grid-cols-3 gap-3">
              <div className="p-3 rounded-xl bg-slate-50">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Remittance</p>
                <p className="text-base font-black text-slate-900 mt-0.5">{formatNGN(remittanceMatch.totalAmount)}</p>
              </div>
              ...
              <div className={cn("p-3 rounded-xl",
                remittanceMatch.netAmount - matchedAmount >= 0 ? "bg-emerald-50" : "bg-rose-50"
              )}>
                <p className={cn("text-[10px] font-black uppercase tracking-widest",
                  remittanceMatch.netAmount - matchedAmount >= 0 ? "text-emerald-600" : "text-rose-600"
                )}>Remaining</p>
                <p className={cn("text-base font-black mt-0.5",
                  remittanceMatch.netAmount - matchedAmount >= 0 ? "text-emerald-600" : "text-rose-600"
                )}>{formatNGN(remittanceMatch.netAmount - matchedAmount)}</p>
              </div>
            </div>
            ...
            <div>
              ...
              {unmatchedClaims.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-slate-400">No approved or settled claims available to match.</p>
                </div>
              ) : (
                <div className="max-h-52 overflow-y-auto space-y-1.5">
                  {unmatchedClaims.map((c) => {
                    const claimTotal = c.amountApproved || c.amountClaimed;
                    ...
                    return (
                      <label key={c.id} className={cn(
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-bold text-slate-900">{c.claimNumber} \u2014 {c.patient ? `${c.patient.firstName} ${c.patient.lastName}` : c.patientId}</p>
                          <p className="text-[10px] text-slate-400 font-medium truncate">{c.hmo?.name || c.hmoId} \u00B7 {c.diagnosis || "N/A"}</p>
                        </div>
            ...
            <div className="flex items-center gap-3 pt-1">
              <Button onClick={() => { setShowMatchModal(null); setSelectedMatchIds(new Set()); }} disabled={matchRemittanceMut.isPending} variant="outline" className="flex-1 h-11 rounded-xl font-bold border-slate-200">Cancel</Button>
              <Button onClick={handleMatch} disabled={selectedMatchIds.size === 0 || matchRemittanceMut.isPending} variant="default" className="flex-1 h-11 rounded-xl font-bold gap-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50">
                {matchRemittanceMut.isPending ? <Loader2 size={16} className="animate-spin" /> : <CheckCircle2 size={16} />}
                {matchRemittanceMut.isPending ? "Matching..." : `Match ${selectedMatchIds.size} Claim${selectedMatchIds.size !== 1 ? "s" : ""}`}
              </Button>
            </div>
          </div>
        )}
