...
import {
  AlertCircle, CheckCircle2, ClipboardList, Clock, Plus, Search,
  ArrowRight, Flag, Trash2, Loader2
} from "lucide-react";
import { motion } from "framer-motion";
import { cn } from "@/lib/utils";
import { Modal } from "@/components/ui/modal";
import { useTasks, useCreateTask, useUpdateTaskStatus, useDeleteTask } from "@/hooks/useStaffTasks";
...
export default function TasksPage() {
  ...
  const filteredTasks = tasks.filter((t: any) => {
    const matchesSearch = !query.trim() || [t.title, t.description, t.id].join(" ").toLowerCase().includes(query.trim().toLowerCase());
  ...
  const handleCreateTask = () => {
    ...
    createTaskMut.mutate({ title: newTask.title, description: newTask.description, priority: newTask.priority }, {
      onSuccess: () => {
  ...
  const handleUpdateStatus = (id: string, status: string) => {
    updateStatusMut.mutate({ id, status }, {
      onSuccess: () => { setShowDetail(false); showToast(`Task updated`); },
    });
  };

  const handleDelete = (id: string) => {
    deleteTaskMut.mutate(id, {
      onSuccess: () => { setShowDetail(false); showToast("Task deleted"); },
    });
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-3">
          {[
            ...
            { label: "Completed", value: String(completedCount), icon: Flag, filter: "completed" },
          ].map((stat) => (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5 sm:gap-8">
        ...
        <section className="xl:col-span-2 bg-white border border-slate-100 rounded-2xl sm:rounded-[3rem] shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-slate-50">
            {isLoading ? (
              <div className="p-8 sm:p-12 text-center"><Loader2 className="animate-spin text-slate-400 mx-auto" /></div>
                ...
                <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-widest truncate">
                      {task.description || "No description"} &bull; {task.dueDate ? new Date(task.dueDate).toLocaleDateString() : "No due date"}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                  <span className={cn("px-2 sm:px-3 py-0.5 rounded-full text-[8px] sm:text-[9px] font-black uppercase tracking-widest", priorityStyles[task.priority] || priorityStyles.medium)}>{task.priority}</span>
                  <span className={cn("px-2 sm:px-3 py-0.5 rounded-full text-[8px] sm:text-[9px] font-black uppercase tracking-widest", statusStyles[task.status] || statusStyles.pending)}>{task.status}</span>
                </div>
              </button>
            ))}
          </div>
        ...
        <aside className="space-y-5 sm:space-y-8">
          <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="space-y-2 sm:space-y-3">
              ...
              <input value={newTask.description} onChange={(e) => setNewTask({ ...newTask, description: e.target.value })} placeholder="Description (optional)"
                className="w-full px-4 sm:px-5 py-3 sm:py-4 rounded-xl sm:rounded-2xl bg-white/10 border border-white/10 focus:outline-none text-sm font-bold placeholder:text-white/30" />
              ...
              <button onClick={handleCreateTask} disabled={!newTask.title.trim() || createTaskMut.isPending}
                className="w-full py-3 sm:py-4 rounded-xl sm:rounded-2xl bg-white text-slate-900 text-[10px] sm:text-xs font-black uppercase tracking-widest hover:bg-sky-50 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
                {createTaskMut.isPending ? <Loader2 size={14} className="animate-spin" /> : null}
          ...
          <div className="space-y-4 sm:space-y-5">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs sm:text-sm text-slate-500 truncate">{selectedTask.dueDate ? new Date(selectedTask.dueDate).toLocaleDateString() : "No due date"}</p>
                  </div>
                  <span className={cn("px-2 sm:px-3 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest shrink-0", priorityStyles[selectedTask.priority] || priorityStyles.medium)}>{selectedTask.priority}</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between p-3 sm:p-4 rounded-lg sm:rounded-2xl bg-slate-50">
                <span className="text-[10px] sm:text-xs font-bold text-slate-400">Description</span>
                <span className="text-xs sm:text-sm font-bold text-slate-900">{selectedTask.description || "—"}</span>
              </div>
              <div className="flex justify-between p-3 sm:p-4 rounded-lg sm:rounded-2xl bg-slate-50">
                <span className="text-[10px] sm:text-xs font-bold text-slate-400">Status</span>
                <span className={cn("px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest", statusStyles[selectedTask.status] || statusStyles.pending)}>{selectedTask.status}</span>
              </div>
            </div>
