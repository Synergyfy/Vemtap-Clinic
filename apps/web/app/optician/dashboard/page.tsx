...
import { ShoppingBag, Cog, PackageSearch, AlertTriangle, CheckCircle2, Truck } from "lucide-react";
import { PageHeader } from "@/app/clinic/_components/page-header";
import { useAuth } from "@/lib/auth-context";
import {
  useOpticianLensOrders,
  useOpticianInventory,
  useOpticianSales,
} from "@/hooks/useOpticianDashboard";
import { cn } from "@/lib/utils";

function statusBadge(status: string) {
  const s = status?.toLowerCase();
  if (s === "pending") return <Badge className="bg-sky-600 text-white">New</Badge>;
  if (s === "processing") return <Badge className="bg-amber-600 text-white">In Production</Badge>;
  ...
  if (s === "pending_sale") return <Badge variant="secondary">Pending</Badge>;
  if (s === "refunded") return <Badge className="bg-rose-600 text-white">Refunded</Badge>;
  if (s === "low") return <Badge className="bg-rose-600 text-white">Low</Badge>;
  if (s === "in_stock") return <Badge className="bg-emerald-600 text-white">In Stock</Badge>;
  if (s === "critical") return <Badge className="bg-rose-600 text-white">Critical</Badge>;
...
export default function OpticianDashboard() {
  ...
  const { data: lensOrders = [], isLoading: ordersLoading } = useOpticianLensOrders(clinicId);
  const { data: inventory = [], isLoading: inventoryLoading } = useOpticianInventory(clinicId);
  const { data: sales = [], isLoading: salesLoading } = useOpticianSales(clinicId);
  ...
  const newOrders = lensOrders.filter((o) => o.status === "pending");
  const inProduction = lensOrders.filter((o) => o.status === "processing");
  const readyForPickup = lensOrders.filter((o) => o.status === "ready");
  const lowStock = inventory.filter((i) => i.quantityInStock <= LOW_STOCK_THRESHOLD);
  const activeProduction = lensOrders.filter((o) => o.status === "processing" || o.status === "pending");

  const pickupPatient = pickupModal ? lensOrders.find((o) => o.id === pickupModal) : null;
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      <PageHeader
        ...
        description="Manage lens orders, production, inventory, and sales."
        actions={[
          { label: "Lens Orders", href: "/optician/orders" },
        ...
        <Card onClick={() => router.push("/optician/orders")} className="hover:border-amber-200 transition-all cursor-pointer group">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-xl font-bold text-slate-900">{lensOrders.length}</p>
            </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:gap-8 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          ...
          <CardContent className="p-0">
            <div className="md:hidden divide-y divide-slate-100">
              {lensOrders.filter((o) => o.status !== "delivered" && o.status !== "cancelled").slice(0, 5).map((o) => (
                <div key={o.id} className="p-4">
                  <div className="flex items-center justify-between gap-3 mb-2">
                    <div className="min-w-0 flex-1">
                      <p className="font-medium text-slate-900 text-sm truncate">{getPatientName(o.patient)}</p>
                      <div className="flex items-center gap-1.5 mt-0.5">
                        <span className="text-[9px] font-mono text-slate-400">{o.id.slice(0, 8)}</span>
                  ...
                  <Link href={`/optician/production?order=${o.id}`} className="inline-flex items-center justify-center rounded-lg bg-amber-600 px-3 py-1.5 text-[10px] font-bold text-white hover:bg-amber-700 w-full mt-2">
                    {o.status === "pending" ? "Start Production" : "Track"}
                  </Link>
                </div>
              ))}
              {lensOrders.length === 0 && !ordersLoading && (
                <p className="text-sm text-slate-500 text-center py-8">No lens orders</p>
              )}
            </div>
            <div className="hidden md:block overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {lensOrders.filter((o) => o.status !== "delivered" && o.status !== "cancelled").slice(0, 5).map((o) => (
                    <TableRow key={o.id}>
                      <TableCell className="text-xs font-mono text-slate-500">{o.id.slice(0, 8)}</TableCell>
                      <TableCell className="font-medium text-slate-900 whitespace-nowrap">{getPatientName(o.patient)}</TableCell>
                  ...
                  {lensOrders.length === 0 && !ordersLoading && (
                    <TableRow>
                      <TableCell colSpan={5} className="text-center text-slate-500">No lens orders</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
        ...
        <Card>
          <CardHeader className="flex-row items-center justify-between px-4 sm:px-6 py-3 sm:py-4">
            ...
            <Link href="/optician/orders" className="text-xs sm:text-sm font-medium text-amber-700 hover:text-amber-800">Full Queue</Link>
          </CardHeader>
          <CardContent className="p-4 sm:p-6 space-y-4">
            {activeProduction.slice(0, 4).map((o) => (
              <div key={o.id} onClick={() => router.push(`/optician/production?order=${o.id}`)} className="rounded-xl border border-slate-200 p-4 hover:border-amber-200 hover:shadow-sm transition-all cursor-pointer">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="font-semibold text-slate-900 truncate">{getPatientName(o.patient)}</p>
                    <p className="mt-0.5 text-xs text-slate-500">{o.lensType}</p>
                  </div>
                  <div className="shrink-0">{statusBadge(o.status)}</div>
                </div>
                <div className="mt-2 flex items-center justify-between text-[10px] text-slate-400">
                  <span>Due: {o.expectedDeliveryDate || "—"}</span>
                  <span>{o.frameDescription || ""}</span>
                </div>
              </div>
            ))}
            {activeProduction.length === 0 && !ordersLoading && (
              <p className="text-sm text-slate-500 text-center py-4">No active production</p>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:gap-8 lg:grid-cols-3">
        <Card>
          ...
          <CardContent className="p-4 sm:p-6 space-y-4">
            {readyForPickup.slice(0, 3).map((o) => (
              <div key={o.id} onClick={() => setPickupModal(o.id)} className="rounded-xl border border-slate-200 p-4 hover:border-emerald-200 hover:shadow-sm transition-all cursor-pointer">
                <p className="font-semibold text-slate-900">{getPatientName(o.patient)}</p>
                <p className="mt-1 text-xs text-slate-500">{o.lensType}</p>
                <div className="mt-3 flex items-center gap-2 text-xs text-emerald-600 font-medium">
                  <CheckCircle2 size={14} />
                  Ready since {o.actualDeliveryDate || o.expectedDeliveryDate || "—"}
                </div>
              </div>
            ))}
            {readyForPickup.length === 0 && !ordersLoading && (
              <p className="text-sm text-slate-500 text-center py-4">Nothing ready for pickup</p>
            )}
          </CardContent>
        </Card>
              ...
              <div key={item.id} onClick={() => router.push("/optician/inventory")} className="rounded-xl border border-slate-200 p-4 hover:border-amber-200 hover:shadow-sm transition-all cursor-pointer">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-xs text-slate-500">{item.brand || ""} &bull; {item.category || ""}</p>
                  </div>
                  <div className="shrink-0">
                    <Badge className={item.quantityInStock === 0 ? "bg-rose-600 text-white" : "bg-amber-600 text-white"}>{item.quantityInStock} left</Badge>
                  </div>
                </div>
                <div className="mt-2 text-xs text-rose-600 font-medium">Low stock &mdash; Reorder needed</div>
              </div>
            ))}
            {lowStock.length === 0 && !inventoryLoading && (
          ...
          <CardContent className="p-4 sm:p-6 space-y-4">
            {sales.slice(0, 3).map((s) => (
              <div key={s.id} onClick={() => setSaleModal(s)} className="rounded-xl border border-slate-200 p-4 hover:border-amber-200 hover:shadow-sm transition-all cursor-pointer">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-semibold text-slate-900">{getPatientName(s.patient)}</p>
                    <p className="mt-0.5 text-xs text-slate-500">{new Date(s.createdAt).toLocaleDateString()} &bull; {s.paymentMethod}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="font-bold text-slate-900">₦{s.totalPrice.toLocaleString()}</p>
            ...
            {sales.length === 0 && !salesLoading && (
              <p className="text-sm text-slate-500 text-center py-4">No sales recorded</p>
            )}
          </CardContent>
        </Card>
      </div>
        ...
        {pickupPatient && (
          <div className="space-y-5">
            <div className="flex items-center gap-4 pb-4 border-b border-slate-100">
              ...
              <div>
                <h4 className="text-lg font-bold text-slate-900">{getPatientName(pickupPatient.patient)}</h4>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button onClick={() => { setPickupModal(null); showToast("Order marked as delivered"); }} className="px-6 py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-bold hover:bg-emerald-700 shadow-lg shadow-emerald-200">Confirm Delivery</button>
            </div>
          </div>
        )}
      </Modal>
        ...
        {saleModal && (
          <div className="space-y-5">
            <div className="flex items-center gap-4 pb-4 border-b border-slate-100">
              ...
              <div>
                <h4 className="text-lg font-bold text-slate-900">{getPatientName(saleModal.patient)}</h4>
                <p className="text-sm text-slate-500">{new Date(saleModal.createdAt).toLocaleDateString()} &bull; {saleModal.paymentMethod}</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-xl bg-slate-50 p-3"><p className="text-[10px] font-bold text-slate-400 uppercase">Total</p><p className="mt-0.5 text-sm font-bold text-slate-900">₦{saleModal.totalPrice.toLocaleString()}</p></div>
              ...
              <div className="rounded-xl bg-slate-50 p-3"><p className="text-[10px] font-bold text-slate-400 uppercase">Sale ID</p><p className="mt-0.5 text-sm font-bold text-slate-900 font-mono">{saleModal.id.slice(0, 8)}</p></div>
