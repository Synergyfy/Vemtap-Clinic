...
import {
  LayoutDashboard, ShoppingBag, PackageSearch, Cog,
  Headphones, Settings2, Bell, Menu, X, LogOut, Eye, BarChart3
} from "lucide-react";
...
const sidebarLinks = [
  { name: "Dashboard", href: "/optician/dashboard", icon: LayoutDashboard },
  { name: "Lens Orders", href: "/optician/orders", icon: ShoppingBag },
  { name: "Inventory", href: "/optician/inventory", icon: PackageSearch },
  { name: "Production", href: "/optician/production", icon: Cog },
  { name: "Optical Sales", href: "/optician/sales", icon: BarChart3 },
  { name: "Support", href: "/optician/support", icon: Headphones },
  { name: "Settings", href: "/optician/settings", icon: Settings2 },
];

export default function OpticianLayout({ children }: { children: React.ReactNode }) {
  ...
  const logout = () => {
    try {
      const keysToRemove = [
        "vemtap:session", "vemtap:clinic", "vemtap:user",
        "vemtap_auth", "authToken", "token",
      ];
  ...
  return (
    <div className="h-screen bg-slate-50 flex overflow-hidden relative">
      ...
      >
        <div className="h-full flex flex-col">
          <div className="p-6 flex items-center justify-between shrink-0 h-20">
            <Link href="/optician/dashboard" className="flex items-center gap-2">
              <div className="w-8 h-8 bg-amber-600 rounded-lg flex items-center justify-center shrink-0">
                <Eye size={20} className="text-white" />
              </div>
              <span className="text-xl font-bold tracking-tight">Optical<span className="text-amber-500">Hub</span></span>
            </Link>
          ...
          <div className="p-4 border-t border-white/10 shrink-0">
            <div className="flex items-center gap-3 p-3 rounded-xl bg-white/5">
              <div className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center text-amber-400 font-bold">
                OS
              </div>
              <div>
                <p className="text-sm font-bold">Opt. S. Danladi</p>
                <p className="text-[10px] text-white/50">Chief Optometrist</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <BottomNav 
        items={[
          { label: "Home", href: "/optician/dashboard", icon: LayoutDashboard },
          { label: "Orders", href: "/optician/orders", icon: ShoppingBag },
          { label: "Inventory", href: "/optician/inventory", icon: PackageSearch },
          { label: "Production", href: "/optician/production", icon: Cog },
        ]}
        activeColor="text-amber-600"
      />

      <Modal isOpen={isLogoutOpen} onClose={() => setIsLogoutOpen(false)} title="Log out">
        <p className="text-sm text-slate-600">Are you sure you want to log out of the optical hub?</p>
