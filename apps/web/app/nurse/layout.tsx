...
import { 
  LayoutDashboard, Users, Heart, Activity,
  Headphones, Settings2, Bell, 
  Menu, X, LogOut, ClipboardList, Eye, Calendar
} from "lucide-react";
...
const sidebarLinks = [
  ...
  { name: "Vitals Entry", href: "/nurse/vitals", icon: Activity },
  { name: "Monitoring", href: "/nurse/monitoring", icon: Heart },
  { name: "Treatment Notes", href: "/nurse/treatment", icon: ClipboardList },
  { name: "Follow-ups", href: "/nurse/followups", icon: Calendar },
  { name: "Eye Tests", href: "/nurse/eye-tests", icon: Eye },
  { name: "Support", href: "/nurse/support", icon: Headphones },
  { name: "Settings", href: "/nurse/settings", icon: Settings2 },
];
...
export default function NurseLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="h-screen bg-slate-50 flex overflow-hidden relative">
      ...
      >
        <div className="h-full flex flex-col">
          <div className="p-6 flex items-center justify-between shrink-0 h-20">
            <Link href="/nurse/dashboard" className="flex items-center gap-2">
              <div className="w-8 h-8 bg-cyan-600 rounded-lg flex items-center justify-center shrink-0">
                <Heart size={20} className="text-white" />
              </div>
              <span className="text-xl font-bold tracking-tight">Nurse<span className="text-cyan-500">Station</span></span>
            </Link>
          ...
          <div className="p-4 border-t border-white/10 shrink-0">
            <div className="flex items-center gap-3 p-3 rounded-xl bg-white/5">
              ...
              <div>
                <p className="text-sm font-bold">Nurse R. Okeke</p>
                <p className="text-[10px] text-white/50">Senior Nursing Officer</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 min-w-0 flex flex-col h-full overflow-hidden">
        <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 z-40">
          ...
          <button className="relative p-2 hover:bg-slate-100 rounded-xl" aria-label="Notifications">
            ...
            <span className="absolute top-2 right-2 w-2 h-2 bg-cyan-500 rounded-full border-2 border-white"></span>
          </button>
          <button
      ...
      <BottomNav 
        items={[
          { label: "Home", href: "/nurse/dashboard", icon: LayoutDashboard },
          { label: "Patients", href: "/nurse/patients", icon: Users },
          { label: "Vitals", href: "/nurse/vitals", icon: Activity },
          { label: "Monitor", href: "/nurse/monitoring", icon: Heart },
        ]}
        activeColor="text-cyan-600"
      />

      <Modal isOpen={isLogoutOpen} onClose={() => setIsLogoutOpen(false)} title="Log out">
        <p className="text-sm text-slate-600">Are you sure you want to log out of the nurse portal?</p>
